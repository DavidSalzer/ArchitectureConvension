...
class EF_Session_Helper {
	...
	public static function fudge_ajax_get_schedule() {
		...
		while ($sessions_loop->have_posts()) {
            ...
            $hosts = wp_get_post_terms(get_the_ID(), 'session-host');
			if ($hosts && count($hosts) > 0)
				$host = $hosts[0];
            else
                $host='';
                $openings = wp_get_post_terms(get_the_ID(), 'session-opening');
			if ($openings && count($openings) > 0)
				$opening = $openings[0];
            else
                $opening='';

                $tracksB = wp_get_post_terms(get_the_ID(), 'session-track');
			if ($tracksB && count($tracksB) > 0)
				$trackB = $tracksB[0];
            else
                $trackB='';
